...
import { toPng } from 'html-to-image' // 👈 [변경] 최신 도구 사용!
...
export default function JiipDetailPage() {
  ...
  // ✍️ [기능 업그레이드] html-to-image 사용 (에러 해결됨!)
  const saveSignature = async () => {
    ...
    try {
        ...
        if (!hiddenContractRef.current) throw new Error("계약서 양식 로드 실패")

        // 3. A4 캡처 -> 이미지 변환 (html-to-image 사용)
        // 👇 이 부분이 핵심입니다! lab 에러 없이 깔끔하게 찍어줍니다.
        ...
        const fileName = `contract_${item.investor_name}_${jiipId}_admin_${Date.now()}.pdf`
        ...
        <div className="mt-8 bg-white p-8 rounded-3xl shadow-sm border border-gray-200">
            ...
            {item.signed_file_url ? (
                <div className="flex flex-col items-center justify-center bg-gray-50 p-6 rounded-xl border border-gray-200">
                    ...
                    <p className="font-bold text-gray-700 mb-4">서명된 계약서(PDF)가 있습니다.</p>
                    <div className="flex gap-4">
                        <a href={item.signed_file_url} target="_blank" className="bg-blue-600 text-white px-4 py-2 rounded-lg font-bold hover:bg-blue-700">파일 열기</a>
