...
export default function JiipDetailPage() {
  ...
  const [item, setItem] = useState<any>({
    ...
    // 공통 정보
    investor_name: '', // 상호 또는 성명
    investor_phone: '', // 연락처
    investor_reg_number: '', // 사업자번호 또는 주민번호
    investor_email: '', // 👈 [신규] 계산서 이메일

    // 주소 (공통 사용)
    investor_address_main: '',
  ...
  const fetchDetail = async () => {
    ...
    if (error) { alert('데이터 로드 실패'); router.push('/jiip'); }
      ...
      setItem({
        ...
        investor_address_detail: '',
        investor_email: data.investor_email || '', // 이메일 불러오기
  ...
  const formatRegNum = (value: string) => {
    ...
    if (item.tax_type === '세금계산서') {
        // 사업자 (3-2-5)
        if (numbers.length > 5) return `${numbers.slice(0, 3)}-${numbers.slice(3, 5)}-${numbers.slice(5, 10)}`
        if (numbers.length > 3) return `${numbers.slice(0, 3)}-${numbers.slice(3)}`
        return numbers
    } else {
        // 주민 (6-7)
        if (numbers.length > 6) return `${numbers.slice(0, 6)}-${numbers.slice(6, 13)}`
        return numbers
    }
  }
      ...
      <div className="space-y-8 bg-white p-8 rounded-3xl shadow-sm border border-gray-200">
          ...
          <div className="bg-blue-50 p-6 rounded-2xl border border-blue-100">
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <label className={`flex items-center gap-3 p-4 rounded-xl border cursor-pointer transition-all ${item.tax_type === '세금계산서' ? 'bg-white border-blue-500 shadow-md ring-2 ring-blue-200' : 'bg-blue-50/50 border-blue-200 hover:bg-white'}`}>
                    <input type="radio" name="tax" value="세금계산서" checked={item.tax_type === '세금계산서'} onChange={e => setItem({...item, tax_type: e.target.value})} className="w-5 h-5 text-blue-600" />
                    <div>
                        <span className="block font-bold text-gray-900">세금계산서 발행</span>
                        <span className="text-xs text-gray-500">사업자 (부가세 별도)</span>
                    </div>
                </label>
                <label className={`flex items-center gap-3 p-4 rounded-xl border cursor-pointer transition-all ${item.tax_type === '사업소득(3.3%)' ? 'bg-white border-blue-500 shadow-md ring-2 ring-blue-200' : 'bg-blue-50/50 border-blue-200 hover:bg-white'}`}>
                    <input type="radio" name="tax" value="사업소득(3.3%)" checked={item.tax_type === '사업소득(3.3%)'} onChange={e => setItem({...item, tax_type: e.target.value})} className="w-5 h-5 text-blue-600" />
                    <div>
                        <span className="block font-bold text-gray-900">사업소득 (3.3%)</span>
                        <span className="text-xs text-gray-500">개인 프리랜서 등</span>
                    </div>
                </label>
                <label className={`flex items-center gap-3 p-4 rounded-xl border cursor-pointer transition-all ${item.tax_type === '이자소득(27.5%)' ? 'bg-white border-blue-500 shadow-md ring-2 ring-blue-200' : 'bg-blue-50/50 border-blue-200 hover:bg-white'}`}>
                    <input type="radio" name="tax" value="이자소득(27.5%)" checked={item.tax_type === '이자소득(27.5%)'} onChange={e => setItem({...item, tax_type: e.target.value})} className="w-5 h-5 text-blue-600" />
                    <div>
                        <span className="block font-bold text-gray-900">이자소득 (27.5%)</span>
                        <span className="text-xs text-gray-500">순수 비영업대금 이익</span>
                    </div>
                </label>
             </div>
          </div>

          {/* 🌟 2. 투자자 정보 (선택에 따라 라벨과 항목이 바뀜) */}
          <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 ...
                 <div>
                    <label className="block text-xs font-bold text-gray-500 mb-1">연락처 (공통)</label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 p-6 rounded-2xl border border-gray-100">
                 ...
                 <div>
                    ...
                    <input className="w-full border p-2 rounded-lg bg-white font-bold"
                        placeholder={item.tax_type === '세금계산서' ? '사업자등록증상 상호' : '신분증상 성명'}
                 ...
                 <div>
                    ...
                    <input className="w-full border p-2 rounded-lg bg-white"
                        placeholder={item.tax_type === '세금계산서' ? '000-00-00000' : '000000-0000000'}
                        maxLength={14}
                 ...
                 <div className="md:col-span-2">
                     <label className="block text-xs font-bold text-gray-500 mb-1">
                        {item.tax_type === '세금계산서' ? '사업장 소재지' : '주민등록상 주소'}
                     </label>
                     <div className="flex gap-2 mb-2">
                        ...
                        <button onClick={handleSearchAddress} className="bg-gray-700 text-white px-3 rounded-lg text-xs font-bold whitespace-nowrap hover:bg-black">주소검색</button>
                     </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                 ...
                 <div className="md:col-span-2">
                    ...
                    <input className="w-full border p-3 rounded-xl bg-white" placeholder="하이픈 없이 숫자만" value={item.account_number} onChange={e => setItem({...item, account_number: e.target.value})} />
                 </div>
              </div>
          </div>
          ...
          {/* 3. 계약 기간 및 투자금 */}
          <div className="space-y-4">
              <h3 className="font-bold text-lg text-gray-900">3. 계약 기간 및 투자금</h3>
          ...
          {/* 4. 수익 배분 조건 */}
          <div className="space-y-4">
              <h3 className="font-bold text-lg text-gray-900">4. 수익 정산 및 배분 조건</h3>
          ...
          <hr className="border-gray-100" />
           ...
           {/* 5. 기타 사항 */}
          <div className="space-y-4">
              <h3 className="font-bold text-lg text-gray-900">5. 기타 사항</h3>
