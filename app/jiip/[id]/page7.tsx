...
export default function JiipDetailPage() {
  ...
  const formatBankAccount = (bank: string, value: string) => {
    const n = value.replace(/[^0-9]/g, "") // 숫자만 추출
    if (!n) return ""

    // 1. KB국민은행 (XXXXXX-XX-XXXXXX 등 다양한데 보통 14자리)
    if (bank === 'KB국민은행') {
        if (n.length > 8) return `${n.slice(0, 6)}-${n.slice(6, 8)}-${n.slice(8, 14)}`
        if (n.length > 6) return `${n.slice(0, 6)}-${n.slice(6)}`
        return n
    }
    // 2. 신한은행 (XXX-XXX-XXXXXX 12자리)
    if (bank === '신한은행') {
        if (n.length > 6) return `${n.slice(0, 3)}-${n.slice(3, 6)}-${n.slice(6, 12)}`
        if (n.length > 3) return `${n.slice(0, 3)}-${n.slice(3)}`
        return n
    }
    // 3. 우리은행 (XXXX-XXX-XXXXXX 13자리)
    if (bank === '우리은행') {
        if (n.length > 7) return `${n.slice(0, 4)}-${n.slice(4, 7)}-${n.slice(7, 13)}`
        if (n.length > 4) return `${n.slice(0, 4)}-${n.slice(4)}`
        return n
    }
    // 4. 하나은행 (XXX-XXXXXX-XXXXX 14자리)
    if (bank === '하나은행') {
        if (n.length > 9) return `${n.slice(0, 3)}-${n.slice(3, 9)}-${n.slice(9, 14)}`
        if (n.length > 3) return `${n.slice(0, 3)}-${n.slice(3)}`
        return n
    }
    // 5. NH농협은행 (XXX-XXXX-XXXX-XX 13자리)
    if (bank === 'NH농협은행') {
        if (n.length > 11) return `${n.slice(0, 3)}-${n.slice(3, 7)}-${n.slice(7, 11)}-${n.slice(11, 13)}`
        if (n.length > 7) return `${n.slice(0, 3)}-${n.slice(3, 7)}-${n.slice(7)}`
        if (n.length > 3) return `${n.slice(0, 3)}-${n.slice(3)}`
        return n
    }
    // 6. 카카오뱅크 (3333-XX-XXXXXXX 13자리)
    if (bank === '카카오뱅크') {
        if (n.length > 6) return `${n.slice(0, 4)}-${n.slice(4, 6)}-${n.slice(6, 13)}`
        if (n.length > 4) return `${n.slice(0, 4)}-${n.slice(4)}`
        return n
    }

    return n // 그 외 은행은 숫자만 표시 (형식이 너무 다양함)
  }
    ...
    <div className="max-w-4xl mx-auto py-10 px-6 animate-fade-in-up pb-32">
      ...
      <div className="space-y-8 bg-white p-8 rounded-3xl shadow-sm border border-gray-200">
          ...
          <div className="space-y-4">
              ...
              <div className="bg-green-50 p-6 rounded-2xl border border-green-100 grid grid-cols-2 gap-6">
                 <div>
